...
# new factorization
feedback_valence <- subset(trial_data, feedback != "neutral")

feedback_valence$feedback <- factor(feedback_valence$feedback, levels = c("positive", "negative"))

# full model
h3_lm_valence <- lmer(sqrt(SoC) ~ feedback * input_noise_magnitude + (1|code), data = feedback_valence)
...
confint(h3_lm, nsim=N_iterations, parm=c('feedbacknegative', 'input_noise_magnitude2', 'feedbacknegative:input_noise_magnitude2'), method='boot')
