...
# Model selection for a linear model predicting performance
...
```
model1.in, the model with input noise as random slope effect wins. Now we can take a look at the main effects.
...
"Increasing input noise magnitude significantly increased the average distance to the line followed throughout a trial (beta={fixef(model1.in)[2]**2}, CI=[0.533456246, 0.63508483],  p<.001).
...
```
We see no significance for several of the effects. This is where we can start. I will target the interactions first to keep the main effects.
...
Processing with model2.1 and trying to eliminate further interaction terms. Here we try to eliminate the interaction between feedback and InternalLC. It also had no significance whatsoever above.
...
We will target our first main effect: InternalLC now:
...
The is no significant main effect of CESDR, but it is involved in a significant interaction effect and it's a numeric variable. We will just leave the effect in the model. This is now our final fixed effects structure. Now we can start to explore random effects structure.
...
```
That took a while and the model is overparameterized (singular)... We will first eliminate interaction effects.
...
Let's compare the nonsingular models and the random intercept only one.
```{r model_comparison, echo=FALSE}
...
...increasing magnitude of input noise significantly decreases SoC (beta={fixef(model2.in)[4]**2}, CI=[-0.364380024, -0.198967385],  p<.001).
